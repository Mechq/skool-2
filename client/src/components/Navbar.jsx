                    ...
                    <ul className="flex flex-col items-center lg:flex-row lg:space-x-8 lg:mt-0 lg:text-sm lg:font-medium font-semibold space-y-0 lg:space-y-0">
                        ...
                        <CustomLink to="/profiel" userRole={userRole}>
                            <div className="flex items-center">
                                <div
                                    className="relative w-8 h-8 overflow-hidden bg-brand-orange-light rounded-full hover:ring-2 hover:ring-brand-orange ring-offset-2">
    ...
    return (
        <li className="relative">
            <button onClick={toggleDropdown}
                    className={`flex items-center py-2 px-3 hover:underline ${isOpen ? 'text-brand-orange' : 'text-gray-900'} rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-brand-orange md:p-0`}>
                {label}
            ...
            {isOpen && (
                <div
                    className="absolute right-0 z-10 mt-2 w-44 origin-top-right rounded-md bg-white shadow-lg">
                    <ul className="py-2 text-sm text-gray-700">
                        {options.map((option, index) => (
                            <li key={index}>
                                <a href={option.href}
                                   className="block px-4 py-2 hover:bg-gray-100">{option.label}</a>
                            </li>
                        ))}
                    </ul>
                </div>
