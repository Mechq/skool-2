...
let commissionController = {
    ...
    getContactPersonByCommissionId: (req, res, next) => {
        const commissionId = req.params.commissionId;
        logger.info("retrieving contact person by commissionId ", commissionId);

        commissionService.getContactPersonByCommissionId(commissionId, (error, success) => {
            if (error) {
                return next({
                    status: error.status,
                    message: error.message,
